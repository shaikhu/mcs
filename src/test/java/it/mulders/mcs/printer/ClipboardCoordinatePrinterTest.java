...
class ClipboardCoordinatePrinterTest implements WithAssertions {
    private static final String COPIED_MESSAGE = "Copied to clipboard";

    private static final String FAILED_MESSAGE = "Failed to copy to clipboard";

    private static final String UNAVAILABLE_MESSAGE = "Clipboard not available";
    ...
    void copies_coordinates_when_clipboard_is_available_and_copy_successful() {
        ...
        assertThat(output).contains(GAV_COORDINATES).contains(COPIED_MESSAGE);
    }
    ...
    void prints_warning_when_copy_fails_even_if_clipboard_is_available() {
        ...
        assertThat(output).contains(GAV_COORDINATES).contains(FAILED_MESSAGE);
    }
    ...
    void prints_warning_clipboard_is_not_available() {
        ...
        assertThat(output).contains(GAV_COORDINATES).contains(UNAVAILABLE_MESSAGE);
    }

    private String getOutput(final ClipboardCoordinatePrinter clipboardPrinter) {
        var out = new ByteArrayOutputStream();
        var stream = new PrintStream(out);
        // CoordinatePrinter doesn't use the query, so null is fine.
